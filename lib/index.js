...
class KarmiaUtilityDate {
    ...
    constructor(options) {
        ...
        if (self.options.date) {
            self.date = _.isDate(self.options.date) ? self.options.date : new Date(self.options.date);
        }
    }
    ...
    setDate(date) {
        const self = this;
        self.date = _.isDate(date) ? date : new Date(date);
...
// Export module
module.exports = KarmiaUtilityDate;
